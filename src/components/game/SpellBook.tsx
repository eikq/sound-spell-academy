          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-3">
              <button onClick={() => onCast?.(s)} aria-label={`Cast ${s.name}`} className="shrink-0">
                <img src={`/assets/ui/icons/spell_${s.id}.png`} alt={`Icon for ${s.name}`} className="size-10 rounded-md border" onError={(e) => { (e.currentTarget as HTMLImageElement).src = '/assets/ui/icons/spell_default.png'; }} />
              </button>
              <div>
                <div className="font-semibold">{s.name}</div>
                <div className="text-xs text-muted-foreground">{s.lore}</div>
              </div>
            </div>
